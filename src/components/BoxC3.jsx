// Reviews Section
import { TeaSVG } from '../assets/TeaSVG';

const BoxC3 = () => (
  <div className="box C3 animated-glass-boxC3">
    <div className="content-C3">
      <div className="C3-title-wrapper">
        <TeaSVG />
        <h1>The Tea</h1>
      </div>

      <div className="tea-container">
        <div className="the-tea tea1">
          <div className="the-tea-title-wrapper">
            <h1>My Balkan Mom</h1>
            <span className="subtitle-C3"><p>Big supporter</p></span>
            <span className="the-tea-stars">⭐⭐⭐⭐⭐ 5/5</span>
          </div>
          <span className="paragraph-C3">
            <p>
              I’ve seen his work, I don’t understand much but every time he creates something,
              it looks absolutely beautiful.
            </p>
          </span>
        </div>
      </div>
    </div>
  </div>
);

export default BoxC3;
